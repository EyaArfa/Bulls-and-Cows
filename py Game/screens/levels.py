...
def easy(music,master):
    master.destroy()
    play(music,min=0,sec=0)
def med(music,master):
    master.destroy()
    play(music,min=2,sec=0)
def hard(music,master):
    master.destroy()
...
def levels(music):
    ...
    #display image
    back=PhotoImage(file=r'py Game\images\bulls-and-cows-android.jpg')
    ...
    btneasy= Button(master,image=ImageTk.PhotoImage(resize_image),borderwidth=0,command=lambda:easy(music,master))
    ...
    btnmed= Button(master,image=ImageTk.PhotoImage(resize_image),borderwidth=0,command=lambda:med(music,master))
    ...
    btnhard= Button(master,image=ImageTk.PhotoImage(resize_image),borderwidth=0,command=lambda:hard(music,master))
